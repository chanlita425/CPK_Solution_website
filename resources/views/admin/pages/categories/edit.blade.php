    ...
    <div class="admin-card p-6">
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name_kh" class="admin-form-input"
                        value="{{ old('name_kh', $category->name_kh) }}" required>
                    @error('name_kh')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    <label class="admin-form-label">Current Icon</label>
                    @if ($category->icon)
                        <img src="{{ asset('storage/' . $category->icon) }}" class="w-20 h-20 object-cover rounded">
                    @else
                ...
                <div>
                    <label class="admin-form-label">New Icon (Optional)</label>
                    <input type="file" name="icon" class="admin-form-input" accept="image/*"
                ...
                <div>
                    ...
                    <select name="is_active" class="admin-form-input">
                        <option value="1" {{ old('is_active', $category->is_active) ? 'selected' : '' }}>Active
                        </option>
                        <option value="0" {{ !old('is_active', $category->is_active) ? 'selected' : '' }}>Inactive
                        </option>
                    </select>
                </div>
            </div>
            ...
            <div class="mt-6 flex justify-end">
                ...
                <button type="submit" class="btn-admin-primary">Update Category</button>
            </div>
        </form>
    </div>
