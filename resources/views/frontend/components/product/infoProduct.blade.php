...
<div id="cart-toast"
    ...
    <span id="cart-toast-msg">{{ __('messages.added_to_cart') }}</span>
    <a href="{{ url('/order') }}" class="ml-2 underline font-bold hover:opacity-80">{{ __('messages.view_cart') }}</a>
</div>
    ...
    function addToCart(productId) {
        ...
        .then(res => {
            if (res.success) {
                ...
                showToast(res.message || '{{ __('messages.failed_to_add') }}', false);
                btn.disabled    = false;
                btn.textContent = '{{ __('messages.add_to_cart') }}';
            }
        })
        .catch(() => {
            showToast('{{ __('messages.something_wrong') }}', false);
            btn.disabled    = false;
            btn.textContent = '{{ __('messages.add_to_cart') }}';
        });
    }
